...
def test_fly_out_set_value_on_instance():
    ...
    assert dummy_func in aw._fly_outs if isinstance(aw._fly_outs, list) else aw._fly_outs == dummy_func
...
def test_fly_out_decorator_on_class():
    ...
    assert getattr(SampleClass, "_fletfly_fly_outs", False) is True
...
def test_fly_out_decorator_on_function():
    ...
    assert getattr(sample_func, "_fletfly_fly_outs", False) is True
...
def test_fly_out_via_instance_attribute_on_function():
    ...
    @aw.fly_out
    def sample_func(page): pass
    assert sample_func in aw._fly_outs
